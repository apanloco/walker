...
5. ~~**Strava credential encryption**~~ — Done. `client_secret`, `access_token`, and `refresh_token` are all encrypted at rest with AES-256-GCM when `WALKER_ENCRYPTION_KEY` is set. Plaintext values in the DB are returned as-is (passthrough) so existing rows are backwards-compatible until the user reconnects Strava.
