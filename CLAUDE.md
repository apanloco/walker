...
5. **Strava token encryption** — `strava_connections` stores access/refresh tokens as plaintext. If DB is compromised, attacker gains read access to connected users' Strava activity data. Mitigation: encrypt at rest with `WALKER_ENCRYPTION_KEY` (AES-GCM). Deprioritised: access tokens expire in 6h, risk is read-only Strava data, consistent with existing UUID-cookie gap (TODO #2).
