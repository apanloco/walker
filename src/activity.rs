...
/// - INIT → WALKING: first step change detected.
/// - INIT → IDLE: impossible. Can't claim idle without first confirming walking.
/// - WALKING → IDLE: no step change for idle timeout.
...
impl ActivityTracker {
    ...
    /// Update with step change from StepTracker.
    pub fn update(
        &mut self,
        ...
        match step_change {
            StepChange::Changed => {
                // Step value changed → WALKING (from any state, including INIT).
                self.phase = ActivityPhase::Walking;
                self.last_step_time = Some(now);
            }
            StepChange::Unchanged => {
                // No change — check idle timeout (only if already confirmed walking).
            ...
            StepChange::Baseline => {
                // First reading after reset — no comparison possible, stay in current phase.
            }
        }
