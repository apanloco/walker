...
Walker connects to your Bluetooth treadmill, detects when you're actually walking (not just standing on the belt), and computes honest calories using peer-reviewed biomechanics models. Data streams to a server that powers a live dashboard with leaderboards, profiles, heatmaps, and history.
...
Walker uses the Ludlow–Weyand (2017) minimum mechanics model for walking and the Minetti (2002) polynomial for running, dispatched at the 7 km/h gait transition. Calories are computed from speed, incline, weight, and duration — never stored, always calculated fresh. This means formula improvements apply retroactively to all historical data.
