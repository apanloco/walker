...
-- Clamped at extremes: below 0.80 km/h → 2.1, above 8.45 km/h → 8.3.
--
...
--   17340  MET 2.1  <1.0 mph          midpoint 0.50 mph = 0.80 km/h
...
DECLARE
  s REAL[] := ARRAY[0.80, 1.61, 2.49, 3.54, 4.35, 5.15, 5.95, 6.76, 7.56, 8.45];
  m REAL[] := ARRAY[2.1,  2.3,  2.8,  3.0,  3.5,  3.8,  4.8,  5.8,  6.8,  8.3];
  i INT;
BEGIN
  IF speed <= s[1] THEN RETURN m[1]; END IF;
  IF speed >= s[10] THEN RETURN m[10]; END IF;
  FOR i IN 1..9 LOOP
    IF speed <= s[i + 1] THEN
